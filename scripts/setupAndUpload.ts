...
async function setupAndUpload() {
  console.log('🚀 Setting up storage buckets and uploading end-goal samples...')
  
  try {
    // First, let's check if the prompts-compiled bucket exists
    console.log('🔍 Checking existing buckets...')
    
    // Try to list objects in prompts-compiled bucket
    ...
    if (listError && listError.message.includes('Bucket not found')) {
      console.log('❌ prompts-compiled bucket not found')
      console.log('📝 Note: Buckets need to be created via Supabase Dashboard or migrations')
      console.log('🔧 Please create the following buckets manually:')
      console.log('   - prompts-compiled (private)')
      console.log('   - prompts (public)')
      console.log('   - tracks (public)')
      console.log('   - tts-audio (private)')
      console.log('\n📋 Or run the migration: supabase/migrations/20250119_create_storage_buckets.sql')
      return
    }
    
    console.log('✅ prompts-compiled bucket exists')
    ...
    console.error('❌ Error in setup and upload:', error)
    process.exit(1)
  }
}
